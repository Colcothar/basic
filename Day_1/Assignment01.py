def process():
  gallon = input ("Please enter the number of gallons of gasoline : ")
  gallon=float(gallon)
  return gallon
gallon = process()  
  
print ("Original number of gallons is : " , gallon)
print (gallon , " gallons is equvivalent of " , gallon*3.7854  , "litres")
print (gallon , " gallons of gasoline requires " , gallon/19.5  , "barrels of oil")
print (gallon , " gallons of gasoline produces " , gallon*20  , "pounds of CO2")
print (gallon , " gallons of gasoline is energy equivalent to " , (gallon*1150)/757  , "gallons of ethanol")
print (gallon , " gallons costs $" , gallon*4.0 , "US dollars"  )
print ("Thanks for playing")


